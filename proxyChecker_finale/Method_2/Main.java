...
public class Main {
    public static void main(String[] args) {
        try {
            FileInputStream fis = new FileInputStream("C://java/ip.txt"); // Создаём поток ввода
            int i;
            String resultIp = "";
            while ((i=fis.read())!=-1){ // Читаем из файла, в конце фала вернётся -1
                if(i == 13) continue; // Символ возврата каретки
                    ...
                    Thread proxyRun = new Thread(new ProxyRun(ip, port) );
                    proxyRun.start();
                    resultIp = "";
...
static class ProxyRun implements Runnable{
    private String ip;
    private int port;
    ProxyRun(String ip, int port){
        this.ip = ip;
        this.port = port;
    }
    @Override
